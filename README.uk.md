...
**Правий клік по чипу** копіює IP сервера, `user@host` або готову команду `ssh`;
адреса в рядку *СЕРВЕР* теж копіюється кліком.
...
**Як налаштувати.** **＋** біля розділу *ПРОЄКТИ* → вкажіть, де лежить тека:

- **на цьому компʼютері** — виберіть її кнопкою **…** (системний діалог) або вставте
  шлях; `compose` працює локальним `docker`;
- **на сервері** — виберіть сервер, а теку знайдіть кнопкою **…** (вбудований оглядач
  по SSH) або впишіть шлях на кшталт `/srv/myapp`; файли, редактор, розмір тек і
  `compose` виконуються на тому хості, а консоль відкривається одразу в цій теці.
  Такий проєкт позначено іконкою сервера.

Проєкт належить серверу, який зараз вибрано: у проду свої теки, у стейджа свої;
«усі сервери» робить локальний проєкт видимим скрізь.
...
- **Змінні оточення** редагуються у вкладці Inspect так само, як у плагіні JetBrains:
  ...
  повертається. Секрети приховані, доки їх не відкрити. Кожна змінна — окремий рядок: кнопки або
  праве меню копіюють назву, справжнє значення (навіть замасковане) чи `KEY=value`;
  подвійний клік копіює значення.
...
| Анімації індикаторів | пульс healthy/unhealthy; вимкнути — ще трохи менше GPU |
